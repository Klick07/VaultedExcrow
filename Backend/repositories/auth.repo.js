...
exports.addUserPassword = async (username, hashedPassword, role) => {
    try {
        const result = await pool.query(`INSERT INTO users (username,hpassword,role) VALUES ($1,$2,$3) RETURNING created_at,is_revoked`, [
            username,
            hashedPassword,
            role
        ]);
...
exports.findUserByUsername = async (username) => {
    const result = await pool.query(`SELECT id,hpassword,is_revoked FROM users WHERE username = $1 LIMIT 1 `, [username]);
...
exports.revokeToken = async(refreshHash)=>{
    const result = await pool.query(`UPDATE refresh_tokens SET revoked = $1  WHERE refresh_hash=$2 RETURNING id`,[true,refreshHash]);
    return result.rows[0]||null;
}
